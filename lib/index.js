...
app.post("/auth", async (req, res) => {
  try {
    ...
    Amplify.configure({
      Auth: {
        Cognito: {
          userPoolId: authConfig.userPoolId?.trim(),
          userPoolClientId: authConfig.userPoolWebClientId?.trim(),
          identityPoolId: authConfig.identityPoolId?.trim(),
          signUpVerificationMethod: "code",
          loginWith: {
            oauth: {
              scopes: authConfig.scope?.map((e) => e.trim()),
              responseType: "token",
            },
          },
        },
    ...
    for (let attempt = 1; attempt <= 10; attempt++) {
      try {
        await signIn({
          ...
          options: {
            authFlowType: authConfig.authenticationFlowType?.trim(),
          },
        });
